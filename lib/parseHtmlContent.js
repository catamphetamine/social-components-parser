import parseHtml from 'html-document-parser/snippet'

import { splitContentIntoBlocksByMultipleLineBreaks } from 'social-components/content'
...
class CommentParser {
	...
	parse(html) {
		...
		const result = this.parseContent(parseHtml(html).childNodes, { topLevel: true, isCode: false })
