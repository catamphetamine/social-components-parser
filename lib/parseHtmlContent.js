 ...
 * @param  {object[]} options.plugins — Plugins for parsing HTML syntax.
 * @param  {object} [options.pluginOptions] — The options that're passed to plugins.
 * @return {any[][]} An array of Content paragraphs.
 */
export default function parseHtmlContent(comment, { plugins, pluginOptions }) {
	return new CommentParser({ plugins, pluginOptions }).parse(comment)
}

class CommentParser {
	constructor({ plugins, pluginOptions = {} }) {
			...
			for (const plugin of this.plugins) {
				if (this.shouldParseUsingPlugin(node, plugin)) {
					...
					return plugin.createBlock(
						...
						plugin.getOptions && plugin.getOptions(this.pluginOptions)
					)
				}
			}
		}
	...
	parseContent(childNodes) {
		...
		while (i < childNodes.length) {
			...
			if (result) {
				...
				// `1` means "DOM element".
				//
				console.warn('[imageboard] Unsupported markup found:', node.nodeType === 1 ? (node.outerHTML || stringifyDomElement(node)) : undefined) // , node)
